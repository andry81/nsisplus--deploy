...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%NSIS_SETUP_DEV.GIT3.ORIGIN%%"
  ) || call :CMD git remote add origin "%%NSIS_SETUP_DEV.GIT3.ORIGIN%%" || ( popd & goto EXIT )
