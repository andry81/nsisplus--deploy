...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%NSISPLUS_DEPLOY.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%NSISPLUS_DEPLOY.GIT.ORIGIN%%" || ( popd & goto EXIT )
